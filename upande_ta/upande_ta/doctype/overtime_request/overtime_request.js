...
frappe.ui.form.on("Overtime Request", {
	...
	get_employees(frm) {
		if (!frm.doc.company || !frm.doc.overtime_date) {
			frappe.msgprint({
				...
				indicator: "red",
				message: __("Set the <b>Company</b> and <b>Overtime Date</b> first."),
			});
			return;
		}
	...
	request_employees(frm, custom_farm) {
		return frappe
			.call({
	...
	show_selection_dialog(frm) {
		...
		let unit = frm._last_unit || null;
		const already = new Set((frm.doc.employees || []).map((row) => row.employee));

		const load = () =>
			frm.events.request_employees(frm, unit).then((result) => {
				...
				dialog.get_field("summary").$wrapper.html(
					result.truncated
						? `<div class="alert alert-warning" style="padding: 8px 12px; margin-bottom: 8px;">${__(
								"Showing the first <b>{0}</b> of <b>{1}</b> employees. Pick a Unit/Division to narrow the list.",
			...
			fields: [
				{
					...
					change() {
						const value = (dialog && dialog.get_value("custom_farm")) || null;
						if (!dialog || value === unit) return;
						unit = value;
						load();
					},
				},
