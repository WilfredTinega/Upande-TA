...
#
# Backported from HRMS v16. A dated, submittable record of which Holiday List
# an Employee (or Company) uses from a given date, so changing a week off keeps
# the previous one visible for earlier dates instead of overwriting history.
...
# Re-use the existing hrms exception so callers (e.g. Bulk Week Off) can catch a
# duplicate assignment. Importing it does not modify hrms.
